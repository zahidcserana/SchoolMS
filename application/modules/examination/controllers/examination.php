...
//        exit;
    ...
    public function selectResult() {
        ...
        $parent_result = $this->db->query("SELECT * FROM result_action WHERE class_id='$class_id' AND status = 'Complete' AND publish = 'Publish'");
        $data['result'] = $parent_result->result_array(); 
        }
        
        if(empty($result)){
            $query = $this->db->query("SELECT class_id FROM class_students WHERE user_id='$userId'");
            $result2= $query->result_array();
             if(!empty($result2)){
             foreach ($result2 as $row) {
            $class_id = $row['class_id'];
        }
        $stu_result = $this->db->query("SELECT * FROM result_action WHERE class_id='$class_id' AND status = 'Complete' AND publish = 'Publish'");
        $data['result'] = $stu_result->result_array();
    ...
    public function sel_ow_ma() {
        if ($this->input->post('submit', TRUE)) {
            if ($this->input->post('examId', TRUE)) {
                ...
                $user = $this->ion_auth->user()->row();
                $userId = $user->id; 
                $studentId = $this->common->student_id($userId); 
