...
class Message extends MX_Controller {
                ...
                if ($group == 'Student') {
                    ...
                    if ($receiver == 'AllStudentSchool') {
                        ...
                        if ($receiver_2 == 'AllStudentsClass') {
                            ...
                            // echo "ss";
                            // echo '<pre>';
                            // print_r($receiver_2); exit; 

                            foreach ($receiver_2 as $key => $value) {
                                ...
                                $message = array(
                                    ...
                                    // echo "single";
                                    //print_r($message); exit; 
                                $resutl = $this->db->insert('massage', $message);
                            }   
    ...
    public function send_message_to_mobile(){ 

        $query = $this->db->query('SELECT * FROM massage where is_message_sent = 0 ');
        $query_result = $query->result_array();

        foreach ($query_result as $key => $value) {
            $receiver_id = $value['receiver_id']; 
            $receiverData = $this->db->query("SELECT * FROM users where id = '$receiver_id' LIMIT 0,1 ");
            $result1 = $receiverData->result_array();


            if(!empty($result1)){
                $phone = $result1[0]['phone'];  
            }else{
                $receiverData2 = $this->db->query("SELECT * FROM student_info where student_id = '$receiver_id' LIMIT 0,1 ");
                $result2 = $receiverData2->result_array();
                $phone = $result2[0]['phone'];  
            }
            
            $message = $this->db->query("SELECT * FROM massage where receiver_id = '$receiver_id' LIMIT 0,1 ");
            $message_result = $message->row();
