...
//                $this->db->insert('student_fee_coll', $feeTableInfo);
                ...
                if ($this->db->insert('student_info', $studentsInfo)) {
                    ...
                    $additionalData3 = array(
                        ...
                        'roll_number' => $this->db->escape_like_str($this->input->post('roll_number', TRUE)),
                        'student_id' => $this->db->escape_like_str($this->input->post('student_id', TRUE)),
                        'stu_machine_id' => $this->db->escape_like_str($this->input->post('admi_stu_machine_id', TRUE)),
                        'class_title' => $this->db->escape_like_str($class_title),
                        'class_id' => $this->db->escape_like_str($class_id),
    ...
    public function student_info() 
    {
		...
		$roll = $this->common->roll_Number_new();
		...
		$check_student_id = $this->common->check_student_id($finalStudentId);
		
		if($check_student_id == TRUE)
		{
			echo '<div class="form-group">
                    <label class="col-md-3 control-label">Student\'s ID <span class="requiredStar"> * </span></label>
                    <div class="col-md-6">
                        <input type="text" name="student_id" class="form-control" value="' . $finalStudentId + 1 . '" readonly>
                    </div>
                </div>';
		}
		else {
        ...
        echo '<div class="form-group">
                    ...
                    <div class="col-md-6">
                        <input data-validation="required" type="text" name="roll_number" class="form-control" value="' . "" . '" onchange="check_roll(this.value)">
                    </div>
                </div>';
    }
	...
	//This function is using for add new student roll check
	function check_roll()
	{
		return $this->common->check_roll();
	}
    ...
    function addParents() 
    {
        if ($this->input->post('submit', TRUE)) 
        {
            ...
            {
                $data['s_class'] = $this->common->getAllData('class');
                $data['success'] = '<div class="col-md-12"><div class="alert alert-warning alert-dismissable admisionSucceassMessageFont">
                                            <button aria-hidden="true" data-dismiss="alert" class="close" type="button"></button>
            ...
            $students = $this->db->query("SELECT * FROM student_info");  
            $studentIds = array();
